...
class NotificationSystem {
    ...
    createNotification(message, type) {
        ...
        notification.className = `
            ...
            max-w-sm w-full ${style.bg} ${style.border} border rounded-lg shadow-lg pointer-events-auto
        `;
            ...
            <div class="p-4">
                <div class="flex items-start">
                    ...
                    <div class="ml-4 flex-shrink-0 flex">
                        <button class="inline-flex ${style.text} hover:text-gray-500 focus:outline-none"
    ...
    showFlashMessages() {
        ...
        Object.entries(flashMessages).forEach(([type, message]) => {
            if (message) {
                this.show(message, type === 'error' ? 'error' : 'success');
            }
        });
        ...
        if (successMeta?.content) {
            this.show(successMeta.content, 'success');
        }
        if (errorMeta?.content) {
            this.show(errorMeta.content, 'error');
        }
        if (warningMeta?.content) {
            this.show(warningMeta.content, 'warning');
        }
        if (infoMeta?.content) {
            this.show(infoMeta.content, 'info');
        }
    }
